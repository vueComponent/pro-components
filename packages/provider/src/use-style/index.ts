...
import { useProProviderContext } from '..'
...
export function useStyle(
  ...
  const info = computed(() => {
    ...
    if (token.value)
      mergedToken.proComponentsCls =
        token.value.proComponentsCls ?? `.${getPrefixCls('pro')}`
    else mergedToken.proComponentsCls = `.${getPrefixCls('pro')}`
  ...
  return {
    wrapSSR: useStyleRegister(info as any, () =>
      styleFn(mergedToken as ProAliasToken)
    ),
    hashId
  }
}
